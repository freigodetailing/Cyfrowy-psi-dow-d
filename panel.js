...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    if (installBtn && !checkIsStandalone()) installBtn.style.display = 'flex';
});
...
document.addEventListener('DOMContentLoaded', () => {
    const installBtn = document.getElementById('installAppBtn');
    if (installBtn && !checkIsStandalone()) {
        // Na iOS zawsze pokazujemy guzik instrukcji, jeśli nie jest zainstalowane
        if (isIOS) {
            installBtn.style.display = 'flex';
        }
    } else if (installBtn && checkIsStandalone()) {
        // Gwarancja, że jeśli jest standalone, to przycisk jest ukryty
