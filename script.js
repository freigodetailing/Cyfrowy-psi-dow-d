...
async function checkOwnershipForMap(dogData) {
    ...
    // Jeśli użytkownik jest właścicielem, pokazujemy przycisk spaceru i dzisiejsze kroki
    if (claim) {
        const walkBtn = document.getElementById('walkSectionBtn');
        if (walkBtn) {
            walkBtn.classList.remove('hidden');
            
            // Pobierz dzisiejsze kroki, aby wyświetlić je na widżecie
            ...
            if (activity && activity.steps > 0) {
                const walkDesc = walkBtn.querySelector('p');
                if (walkDesc) walkDesc.innerHTML = `Dziś zrobiliście: ${activity.steps} kroków`;
            }
