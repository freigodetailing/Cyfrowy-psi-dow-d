...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function initStepCounter(mainPid) {
        ...
        function startWalk() {
            ...
            // Natychmiastowe pokazanie powiadomienia
            if (navigator.serviceWorker && navigator.serviceWorker.controller) {
                navigator.serviceWorker.controller.postMessage({ 
                    type: 'SHOW_WALK_NOTIFICATION', 
                    steps: totalDailySteps, 
                    duration: '00:00:00', 
                    petName: document.getElementById('miniPetName').textContent 
                });
            }
            ...
            walkTimerInterval = setInterval(() => {
                ...
                const timeStr = `${h}:${m}:${s}`;
                durationText.textContent = timeStr;
        ...
        function handleMotion(event) {
            ...
            // Lepszy filtr wygładzający
            smoothMag = (smoothMag * 0.8) + (rawMag * 0.2);
            
            // Obniżony próg dla lepszej czułości na mobile (11.4 zamiast 11.8)
            const threshold = 11.4;
            const now = Date.now();
            
            if (smoothMag > threshold && lastAccelMag <= threshold && (now - lastStepTime) > 300) {
                lastStepTime = now;
                stepsSinceLastSync++;
