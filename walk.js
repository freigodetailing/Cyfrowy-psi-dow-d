...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function initStepCounter(mainPid) {
        ...
        function startWalk() {
            ...
            walkTimerInterval = setInterval(updateWalkDuration, 1000);
            window.addEventListener('devicemotion', handleMotion);
            
            // Ukryj możliwość zmiany psów w trakcie
        ...
        async function stopWalk() {
            ...
            // Synchronizacja wszystkich wybranych psów
...
async function renderActivityChart(petId) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        wrapper.innerHTML = `<div class="chart-bar ${i === 6 ? 'current' : ''}" style="height: ${height}%" title="${steps} kroków"></div><span class="chart-day">${dayName}</span>`;
